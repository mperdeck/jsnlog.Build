...
Function ApplyVersion([string]$templatePath)
{
	...
	(Get-Content $templatePath) | Foreach-Object {$_ -replace $versionPlaceholder, $currentCoreVersion} | Out-File -encoding ascii $filePath
	...
	(Get-Content $templatePath) | Foreach-Object {$_ -replace $JSNLogJsVersionPlaceholder, $currentJSNLogJsVersion} | Out-File -encoding ascii $filePath
